<?php
  ...
  $stmt = $pdo->prepare("SELECT * FROM salary,employee WHERE employee.emp_id = salary.emp_id AND employee.emp_permission = 0");
  $stmt->execute(); 
?>
<!DOCTYPE html>
<html>
<head>
	<title>emp_salary</title>
	<meta charset="utf-8">
      ...
      <div style="margin-right: 150px;padding-top: 20px;padding-left: 50px; padding-right: 5px;">
        ...
        <table class='table table-hover table-dark' style='box-shadow: 2px 2px 50px red;'>
          ...
          <tr>
            ...
            <td><?=$row["emp_rank"]?></td>
            <td><?=$row["salary_time"]?> ชั่วโมง.</td>
		        <td><?=$row["salary_receive"]?> บาท.</td>
          </tr>
        <?php } ?>
        </table>
      </div>
