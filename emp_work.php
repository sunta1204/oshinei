...
$stmt = $pdo->prepare("SELECT * FROM employee WHERE emp_permission = 0 ");
?>
<!DOCTYPE html>
<html>
<head>
<title>emp_work</title>
<meta charset="utf-8">
    ...
    <div class="container" style="background-color: rgb(180, 180, 180); box-shadow: 0 50px 70px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19); height: 550px;box-shadow: 2px 2px 200px red;">  
        <nav style="float: left;width: 200px;height: 400px;margin-top: 20px;margin-bottom: 5px;">
            <nav class="w3-panel w3-border-white w3-leftbar w3-rightbar w3-topbar w3-bottombar" ><br>
              ...
              <a href="department.php"><input type="button" value="รายชื่อแผนก" class="btn btn-primary form-control"></a><br><br>
        ...
        <article style="margin-left: 200px">
          <div style="margin-right: 50px;padding-top: 20px;padding-left: 50px; padding-right: 5px;">
            <h1 style="text-align: left; size: 60px; color: white; text-shadow: 2px 2px 5px red">ตารางเช็คเวลาเข้างานพนักงาน</h1><br>
              <table class='table table-hover table-dark' style="box-shadow: 2px 2px 50px red;">
                <tr>
                  <th>รหัสพนักงาน</th>
                  <th>ชื่อ-นามสกุล</th>
                  <th>มาเข้างานเต็มวัน</th>
                  <th>ไม่มาทำงาน</th>
                  <th>OT</th>
                </tr>
                  <?php
                ...
                <tr>
                  ...
                  <td><a class="btn btn-primary" href="check-work/check-work.php?emp_id=<?=$row["emp_id"] ?>"> มาเข้างาน</a>
                      <input type="hidden" name="emp_id" value="<?=$row["emp_id"]?>">
                  </td>
              	  <td><a class="btn btn-danger"  href="check-absence/check-absence.php?emp_id=<?=$row["emp_id"]?>"> ขาดงาน </a>
              		    <input type="hidden" name="emp_id" value="<?=$row["emp_id"]?>">
              	  </td>
                  <td>
                    <div class="btn-group" role="group">
                      <button id="btnGroupDrop1" type="button" class="btn btn-secondary dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                      เลือกจำนวนชั่วโมง</button>
                      <div class="dropdown-menu btn btn-primary" aria-labelledby="btnGroupDrop1" >
                        <a class="dropdown-item" href="ot/ot1h.php?emp_id=<?=$row["emp_id"] ?>">ทำ OT 1 ชม.</a>
                        <input type="hidden" name="emp_id" value="<?=$row["emp_id"]?>">
                        
                        <a class="dropdown-item" href="ot/ot2h.php?emp_id=<?=$row["emp_id"] ?>">ทำ OT 2 ชม.</a>
                        <input type="hidden" name="emp_id" value="<?=$row["emp_id"]?>">
                        
                        <a class="dropdown-item" href="ot/ot3h.php?emp_id=<?=$row["emp_id"] ?>">ทำ OT 3 ชม.</a>
                        <input type="hidden" name="emp_id" value="<?=$row["emp_id"]?>">
                        
                        <a class="dropdown-item" href="ot/ot4h.php?emp_id=<?=$row["emp_id"] ?>">ทำ OT 4 ชม.</a>
                        <input type="hidden" name="emp_id" value="<?=$row["emp_id"]?>">
                      </div>
                    </div>
                  </td>
