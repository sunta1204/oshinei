...
<body>
  <!-- navbar -->
	...
	<div class="container" style="background-color: rgb(180, 180, 180); box-shadow: 0 50px 70px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19); height: 720px;box-shadow: 2px 2px 200px red;">  
    <nav style="float: left;width: 200px;height: 700px;margin-top: 20px;margin-bottom: 5px;">
      
      <nav class="w3-panel w3-border-white w3-leftbar w3-rightbar w3-topbar w3-bottombar" ><br>
        <a href="edit_form.php?emp_id=<?=$_SESSION["emp_id"]?>"><input type="button" value="แก้ไขข้อมูลส่วนตัว" class="btn btn-primary form-control"></a><br><br>
    ...
    <!-- Start show employee and salary table -->
      ...
      <h1 style="text-align: left; size: 60px;color: white; text-shadow: 2px 2px 5px red;">รายละเอียด</h1><br>
          ...
          <tr>
            <td><?=$_SESSION["emp_id"]?></td>
		        <td><?=$_SESSION["emp_fullname"]?></td>
            <td><?=$_SESSION["emp_year"]?></td>
            <td><?=$_SESSION["emp_sex"]?></td>
		        <td><?=$_SESSION["emp_cardid"]?></td>
          </tr>    
		    </table><br>
		    <table class='table table-hover table-dark' style='box-shadow: 2px 2px 50px red;'>
      	  <tr>
            <th>ตำแหน่ง</th>
            <th>เวลาการทำงาน</th>
            <th>รายได้ ณ ปัจจุบัน</th>
          </tr>
          <tr>
            <td><?=$_SESSION["emp_rank"]?></td>
            <td><?=$_SESSION["salary_time"]?> ชั่วโมง.</td>
		        <td><?=$_SESSION["salary_receive"]?> บาท.</td>
          </tr>       
        </table><br>
        <table class='table table-hover table-dark' style='box-shadow: 2px 2px 50px red;'>
          <tr>
          ...
          <tr>
            <td><?=$_SESSION["emp_email"]?></td>
            <td><?=$_SESSION["emp_address"]?></td>
          </tr>     
        </table> <br>    
      </div>
    </article>
  ...
  <!-- END show employee and salary table -->
